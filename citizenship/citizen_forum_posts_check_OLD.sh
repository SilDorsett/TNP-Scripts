...
for row in "${rows[@]}"; do
   ...
   if [ $cit_compliance -gt 0 ]; then
      ...
      if [ $fail_region -gt 0 ]; then
         echo "<br>$tnp_nation is NOT IN THE REGION." >> $report_file
      fi
      if [ $fail_forum -gt 0 ]; then
         echo "<br>$forum_account HAS NOT POSTED ON THE FORUM in 30 Days." >> $report_file
      fi
      if [ $fail_rmb -gt 0 ]; then
        echo "<br>$forum_account HAS NOT POSTED ON THE RMB in 30 Days." >> $report_file
      fi

      echo "<br>   Review the following:" >> $report_file
      echo "<br>   Forum Account : <a href=\"https://forum.thenorthpacific.org/profile/${profile_id}/#recent-content\">profile/${profile_id}</a>" >> $report_file
      echo "<Br>   NationStates  : <a href=\"https://www.nationstates.net/page=rmb_search?rmbsearch-text=&rmbsearch-author=${tnp_nation}&rmbsearch-region=The+North+Pacific&rmbsearch-sort=new\">nation=${tnp_nation}</a>" >> $report_file
